...
use crate::ai::generate::GenerateResult;
...
fn enhance_with_ai(layout: &RepoLayout, raw_body: &str) -> CoreResult<String> {
    let provider = super::generate::build_ai_provider(layout, None)?;
    let system = crate::ai::prompt::system_prompt(ArtifactKind::ImplementationPrompt);
    let result = provider
