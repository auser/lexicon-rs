use crate::api::diff::ApiDiff;
use crate::audit::writer::write_audit_record;
use crate::coverage::report::CoverageReport;
use crate::gates::result::GateOutcome;
use crate::gates::runner::run_all_gates;
use crate::repo::layout::RepoLayout;
use crate::scoring::engine::{compute_score, DimensionResult, ScoreReport};
use crate::spec::audit::AuditRecord;
use crate::spec::common::{Actor, AuditAction, DimensionCategory};
...
pub fn verify(layout: &RepoLayout) -> CoreResult<VerifyResult> {
    ...
    let coverage_report = if !contracts.is_empty() {
        crate::core::coverage::coverage_report(layout, &contracts).ok()
    } else {
        None
    };

    // API drift check
    let api_diff = if crate::core::api::has_baseline(layout) {
        // Scan current API first
        let _ = crate::core::api::api_scan(layout);
        crate::core::api::api_diff(layout).ok()
    } else {
        None
    };
    ...
    let prompt_warnings = match crate::core::prompt_graph::load_graph(layout) {
        Ok(graph) => match crate::core::prompt_graph::find_dirty_sources(layout, &graph) {
            Ok(dirty) if !dirty.is_empty() => {
                let affected = crate::core::prompt_graph::find_affected_prompts(&graph, &dirty);
                affected
                    .iter()
...
fn map_gate_results_to_dimensions(
    model: &ScoreModel,
    gate_results: &[crate::gates::result::GateResult],
...
fn coverage_dimension(
    dim: &crate::spec::scoring::ScoreDimension,
