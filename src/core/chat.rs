...
use crate::ai::generate::load_context_selective;
use crate::ai::prompt::{build_chat_user_message, estimate_tokens, CHAT_SCHEMAS, CHAT_SYSTEM};
use crate::conversation::driver::ConversationDriver;
use crate::conversation::session::{list_sessions, save_session};
use crate::conversation::workflow::Question;
...
use super::generate::build_ai_provider;
...
fn execute_create_contract(
    ...
    let contract: crate::spec::contract::Contract =
...
fn execute_create_gate(
    ...
    // Count gates
    let model: Result<crate::spec::gates::GatesModel, _> = toml::from_str(toml_content);
...
fn execute_create_conformance(
    ...
    let result = super::generate::generate_from_intent(layout, ArtifactKind::Conformance, &intent)?;
    super::generate::accept_artifact(layout, &result.artifact)?;
...
fn execute_create_behavior(
    ...
    let result = super::generate::generate_from_intent(layout, ArtifactKind::Behavior, &intent)?;
    super::generate::accept_artifact(layout, &result.artifact)?;
...
fn execute_generate_prompt(
    ...
    for contract_id in &contract_ids {
        match super::prompt_gen::generate_prompt(layout, contract_id, None, false) {
            Ok(result) => {
                super::generate::accept_artifact(layout, &result.artifact)?;
                let path = result.artifact.path.clone();
                ctx.artifacts.push(SessionArtifact {
...
fn execute_verify(layout: &RepoLayout) -> CoreResult<String> {
    match super::verify::verify(layout) {
        Ok(result) => {
...
fn execute_create_property_tests(
    ...
    let result = super::generate::generate_contract_property_tests(layout, &contract)?;
    super::generate::accept_artifact(layout, &result.artifact)?;
...
fn execute_create_fuzz_target(
    ...
    let contract = load_contract_by_id(layout, contract_id)?;
    let result = super::generate::generate_contract_fuzz_target(layout, &contract)?;
    super::generate::accept_artifact(layout, &result.artifact)?;
...
fn execute_create_edge_cases(
    ...
    let contract = load_contract_by_id(layout, contract_id)?;
    let result = super::generate::generate_contract_edge_case_tests(layout, &contract)?;
    super::generate::accept_artifact(layout, &result.artifact)?;
...
fn execute_infer_contract(
    ...
    let result = super::generate::infer_contract_from_api(layout, None)?;
    super::generate::accept_artifact(layout, &result.artifact)?;

    let path = result.artifact.path.clone();
    // Try to extract ID from the generated artifact content
    let id = if let Ok(contract) = toml::from_str::<crate::spec::contract::Contract>(&result.artifact.content) {
...
fn execute_api_baseline(layout: &RepoLayout) -> CoreResult<String> {
    ...
    if !layout.api_dir().join("current.json").exists() {
        super::api::api_scan(layout)?;
    }
    super::api::api_baseline(layout)?;
    Ok("API baseline saved".to_string())
}

fn execute_sync_claude(layout: &RepoLayout) -> CoreResult<String> {
    super::sync_claude::sync_claude(layout)?;
...
fn execute_doctor(layout: &RepoLayout) -> CoreResult<String> {
    ...
    let contracts = super::contract::contract_list(layout)?;
...
fn execute_prompt_list(layout: &RepoLayout) -> CoreResult<String> {
    let prompts = super::prompt_gen::list_prompts(layout)?;
    if prompts.is_empty() {
...
fn execute_prompt_status(layout: &RepoLayout) -> CoreResult<String> {
    let statuses = super::prompt_gen::check_all_prompt_statuses(layout)?;
...
fn execute_regenerate_prompts(layout: &RepoLayout) -> CoreResult<String> {
    let results = super::prompt_gen::regenerate_stale(layout, false)?;
    if results.is_empty() {
...
fn run_verify_inline(layout: &RepoLayout, dim_style: &Style) -> CoreResult<()> {
    ...
    let result = super::verify::verify(layout)?;
    ...
    if let Some(ref score) = result.score_report {
        ...
        let verdict_style = match score.verdict {
            crate::scoring::engine::Verdict::Pass => Style::new().green().bold(),
            crate::scoring::engine::Verdict::Warn => Style::new().yellow().bold(),
            crate::scoring::engine::Verdict::Fail => Style::new().red().bold(),
        };
        println!(
            "  Score: {} ({:?})",
