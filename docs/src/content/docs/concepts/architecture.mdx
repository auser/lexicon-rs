...
Lexicon is a Rust workspace of 12 crates organized into strict dependency layers.
...
Layer 3 (domain)    conversation  scaffold
                       |    |      |   |
Layer 2 (services)   repo  audit  gates
                       |     |   /   |
Layer 1 (engines)     fs  scoring  conformance
                       \     |     /
Layer 0 (types)         \  spec  /
```
...
**Layer 1 — `fs`, `scoring`, `conformance`**: Low-level engines with no
cross-dependencies. `fs` handles atomic file I/O, `scoring` computes weighted
scores, `conformance` checks structural compliance.

**Layer 2 — `repo`, `audit`, `gates`**: Repository layout discovery, audit
trail persistence, and gate execution (subprocess runner).
