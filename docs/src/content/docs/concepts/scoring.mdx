...
The core idea: **make scoring functions concrete**. The score system is deterministic and explainable -- you should always be able to understand exactly why the score is what it is. There is no hidden heuristic. Run `lexicon score explain` and the system tells you precisely which dimensions contributed what.
...
- `lexicon score init` -- Create the scoring model through a conversational workflow
- `lexicon score explain` -- Display the current model with a full breakdown of why the score is what it is
