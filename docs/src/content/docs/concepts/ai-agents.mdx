...
This context is assembled from contracts, scoring models, gates, and policy into a structured summary. The `lexicon sync claude` command writes this into `CLAUDE.md` as managed blocks that stay current with the repo state.
...
Lexicon supports generating artifacts from natural language descriptions:
...
4. Presents a patch preview
5. Writes to disk only after explicit user approval

All generated artifacts follow the same schemas as manually-created ones. Every acceptance or rejection is recorded in the audit trail.
...
This is not an uncontrolled auto-rewrite feature. It is bounded and auditable. The AI can propose improvements -- adding missing tests, strengthening conformance harnesses, updating contracts, documenting new API behavior -- but it must not silently weaken contracts or remove tests.
