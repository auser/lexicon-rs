...
fn run_login(provider: Option<Provider>, port: Option<u16>) -> miette::Result<()> {
    ...
    let _creds = lexicon_core::auth::login(&layout, provider, port)?;
...
fn run_refresh(provider: Option<Provider>) -> miette::Result<()> {
    ...
    let provider = match provider {
        Some(p) => p,
        None => {
            let results = lexicon_core::auth::status(&layout)?;
    ...
    let _refreshed = lexicon_core::auth::refresh(&layout, provider)?;
...
fn run_logout(provider: Option<Provider>) -> miette::Result<()> {
    ...
    lexicon_core::auth::remove(&layout, provider)?;
