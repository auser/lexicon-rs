...
use lexicon_conversation::driver::{ConversationDriver, ProposalResponse};
use lexicon_conversation::session::save_session;
use lexicon_conversation::workflow::{Proposal, Question};
...
use crate::generate::{accept_artifact, generate_from_intent, refine_from_intent, reject_artifact};
...
fn load_contract(
    layout: &RepoLayout,
    contract_id: &str,
) -> CoreResult<lexicon_spec::contract::Contract> {
    let path = layout.contracts_dir().join(format!("{contract_id}.toml"));
    let text = std::fs::read_to_string(&path).map_err(|_| {
        crate::error::CoreError::Other(format!("Contract not found: {contract_id}"))
    })?;
    let contract: lexicon_spec::contract::Contract = toml::from_str(&text)
        .map_err(|e| crate::error::CoreError::Other(format!("Failed to parse contract: {e}")))?;
    Ok(contract)
}
...
fn update_prompt_graph_if_needed(layout: &RepoLayout, driver: &dyn ConversationDriver) {
    if let Ok(mut graph) = crate::prompt_graph::load_graph(layout) {
        if let Ok(nodes) = crate::prompt_graph::discover_source_nodes(layout) {
            for node in nodes {
                crate::prompt_graph::upsert_node(&mut graph, node);
            }
            let _ = crate::prompt_graph::save_graph(layout, &graph);
        }

        // Check for stale prompts
        if let Ok(dirty) = crate::prompt_graph::find_dirty_sources(layout, &graph) {
            if !dirty.is_empty() {
